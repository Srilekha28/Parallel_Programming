...
int main( int argc, char *argv[] ) {
  ...
  //for ( int t = 1; t < max_time ; t++) {
    ...
    for (i = 0 ; i < size ; i++) {
       for (j = 0 ; j < size ; j++) {
            z[1][i][j] = z[0][i][j] + pow(2,c)/2 * (pow(2,(dt/dd))) * (z[0][i+1][j] + z[0][i-1][j] + z[0][i][j+1] + z[0][i][j-1] - 4.0 * z[0][i][j];
        }
     }
  ...
  // simulate wave diffusion from time >= 2
    // IMPLEMENT BY YOURSELF !!!
  for ( t = 2; t < max_time; t++ ) {
    for ( i = 0 ; i < size ; i++) {
       for( j = 1; j < size ; j++) {
          z[t%3][i][j] = 2.0 * z[1][i][j] - z[0][i][j] + pow(2,c)*(pow(2,(dt/dd))) * (z[1][i+1][j] + z[1][i-1][j] + z[1][i][j+1] + z[1][i][j-1] - 4.0 * z[1][i][j];
       }
    }
  } // end of simulation
